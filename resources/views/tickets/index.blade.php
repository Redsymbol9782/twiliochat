...
@section('content')
    ...
    @can('ticket_create')
    <p>
        <a href="{{ route('tickets.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
                    ...
                    @if (count($tickets) > 0)
                        @foreach ($tickets as $ticket)
                            <tr data-entry-id="{{ $ticket->id }}">
                                ...
                                <td>
                                    @can('ticket_view')
                                    <a href="{{ route('tickets.show',[$ticket->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('ticket_edit')
                                    <a href="{{ route('tickets.edit',[$ticket->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('ticket_delete')
                                    {!! Form::open(array(
